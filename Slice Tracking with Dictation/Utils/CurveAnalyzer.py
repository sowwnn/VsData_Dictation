...
class CurveAnalyzer:
    ...
    def sliding_window_classify(self):
        ...
        # segments = self.resolve_overlap_and_merge(segments)
        self.classified_segments = segments
        return segments

    def resolve_overlap_and_merge(self, segments):
        # Ưu tiên: 4=2 > 5 > 1 > 3
        class_priority = {4: 3, 2: 3, 5: 5, 1: 1, 3: 4}
        segments = sorted(segments, key=lambda x: (x['start_time'], -class_priority[x['class']]))
        result = []
        for seg in segments:
            if not result:
                result.append(seg)
            ...
            if seg['start_time'] < last['end_time']:
                # Nếu class mới ưu tiên hơn
                if class_priority[seg['class']] > class_priority[last['class']]:
                    ...
                    if last['end_time'] > last['start_time']:
                        ...
                        result[-1] = seg
                # Nếu class cũ ưu tiên hơn, bỏ qua seg mới
                continue
            else:
                result.append(seg)
        # Gộp các đoạn cùng class liền kề (nếu không cách nhau quá xa)
        merged = [result[0].copy()]
        for seg in result[1:]:
            last = merged[-1]
            if seg['class'] == last['class'] and seg['start_time'] <= last['end_time'] + 4:
                merged[-1]['end_time'] = max(last['end_time'], seg['end_time'])
            else:
                merged.append(seg.copy())
        return merged
...
def test():
    ...
    analyzer.load_tracking_csv("test/tracking4.csv")
